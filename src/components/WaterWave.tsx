...
export default function WaterWave({ currentIntake, dailyGoal }: WaterWaveProps) {
  ...
  return (
    <div id="water-wave-container" className="flex flex-col items-center">
      ...
      <div 
        id="glass-vessel" 
        className="relative w-64 h-80 rounded-b-[40px] rounded-t-[16px] border-4 border-slate-200/80 bg-slate-50/10 shadow-[inset_0_4px_12px_rgba(255,255,255,0.4),0_12px_24px_-8px_rgba(14,165,233,0.15)] overflow-hidden flex items-end backdrop-blur-sm"
      >
        ...
        <motion.div 
          id="liquid-fill"
          className="absolute left-0 right-0 bottom-0 bg-gradient-to-t from-sky-500 to-sky-400"
          ...
          {percentage > 0 && percentage < 100 && (
            <div className="absolute w-[200%] h-12 -top-10 left-0">
              <svg 
                ...
                className="absolute w-full h-full fill-sky-400 opacity-60 animate-[wave_12s_infinite_linear]"
              >
              ...
              <svg 
                viewBox="0 0 1200 120" 
                preserveAspectRatio="none" 
                className="absolute w-full h-full fill-sky-300 opacity-40 animate-[wave_7s_infinite_linear] left-[-50%]"
              >
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 text-center select-none pointer-events-none">
          {isGoalReached ? (
            <motion.div
              ...
              className="flex flex-col items-center p-3 rounded-2xl bg-white/40 backdrop-blur-md shadow-lg border border-white/50"
            >
              <Trophy className="w-10 h-10 text-amber-500 animate-bounce" />
              <span className="text-amber-800 font-sans font-bold text-sm uppercase tracking-wider mt-1">Goal Met!</span>
              <span className="text-slate-800 font-mono text-xs mt-0.5">{currentIntake} / {dailyGoal} mL</span>
            </motion.div>
          ) : (
            <div className="flex flex-col items-center drop-shadow-md">
              <span className="text-3xl font-sans font-bold text-slate-800 tabular-nums">
                {percentage.toFixed(0)}%
              </span>
              <span className="text-xs font-sans font-semibold text-slate-600 tracking-medium uppercase mt-0.5">
